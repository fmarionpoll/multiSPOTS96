...
/**
 * Modern, thread-safe implementation of cage management with clean code practices.
 * 
 * <p>This class provides comprehensive functionality for managing individual cages:
 * <ul>
 ...
 * <p>Usage example:
 * <pre>{@code
 * ModernCage cage = ModernCage.builder()
 *     .withData(CageData.createValid(roi, properties))
 *     .build();
 * 
...
public final class ModernCage implements Comparable<ModernCage>, AutoCloseable {
    ...
    /**
     * Adds an elliptical spot to this cage with validation and rich error reporting.
     * 
    ...
    public CageOperationResult addSpot(Point2D.Double center, int radius) {
        ...
        try {
            // Input validation
            if (center == null) {
                return CageOperationResult.failure("ADD_SPOT", 
                    new IllegalArgumentException("Center cannot be null"), 
                    "Invalid spot center");
            }
            ...
            // Create spot
            Spot spot = createEllipseSpot(spotsArray.spotsList.size(), center, radius);
            ...
            return CageOperationResult.success("ADD_SPOT", "Successfully added spot")
                .toBuilder()
                .processingTimeMs(processingTime)
                .addMetadata("spotCount", spotsArray.spotsList.size())
                .addMetadata("spotRadius", radius)
                .addMetadata("spotCenter", center)
                .build();
    ...
    public CageOperationResult computeMask() {
        ...
        try {
            ...
            return CageOperationResult.success("COMPUTE_MASK", "Successfully computed cage mask")
                .toBuilder()
                .processingTimeMs(processingTime)
                .addMetadata("maskPoints", mask.getNumberOfPoints())
                .build();
    ...
    public CageOperationResult clearMeasures() {
        ...
        try {
            ...
            return CageOperationResult.success("CLEAR_MEASURES", "Successfully cleared all measures")
                .toBuilder()
                .processingTimeMs(processingTime)
                .build();
    ...
    public CageOperationResult validateCage() {
        ...
        try {
            ...
            if (isValid) {
                return CageOperationResult.success("VALIDATE", "Cage validation passed")
                    .toBuilder()
                    .processingTimeMs(processingTime)
                    .addMetadata("cageName", name)
                    .addMetadata("cageID", data.getProperties().cageID)
                    .build();
            } else {
                return CageOperationResult.failure("VALIDATE", 
    ...
    public String toString() {
        return String.format("ModernCage{name='%s', id=%d, valid=%b, spots=%d}", 
                           data.getName(), data.getProperties().cageID, data.isValid(), 
                           spotsArray.spotsList.size());
    }
    ...
    private Spot createEllipseSpot(int position, Point2D.Double center, int radius) {
        ...
        spot.prop.cageID = data.getProperties().cageID;
        spot.prop.cagePosition = position;
