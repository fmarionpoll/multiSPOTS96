...
public class Display extends JPanel implements ViewerListener {
	...
	void displayOFF() {
		...
		if (exp == null || exp.seqKymos == null)
			return;
		ArrayList<Viewer> vList = exp.seqKymos.getSequence().getViewers();
		if (vList.size() > 0) {
			for (Viewer v : vList)
				v.close();
			vList.clear();
...
//		}
	...
	private void changeBinSubdirectory(String localString) {
		...
		exp.seqKymos.getSequence().close();
		exp.zloadKymographs();
	...
	private void updateTransformFunctionsOfCanvas(Experiment exp) {
		Viewer viewer = exp.seqKymos.getSequence().getFirstViewer();
		if (viewer != null) {
