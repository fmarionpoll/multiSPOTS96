...
/**
 * Utility class for performing K-means clustering on image data.
 * This class provides functionality to segment images using K-means
 * clustering algorithm with various parameters for color space and
 * clustering configuration.
 * 
 * <p>ImageKMeans is used in the MultiSPOTS96 plugin for image
 * segmentation and analysis, particularly for identifying regions
 * of interest in experimental data.</p>
 * 
 * <p>Usage example:
 * <pre>
...
public class ImageKMeans {
     ...
     */
    public static Segmentation doClustering(Sequence seq)
            throws SupportRegionException, SegmentationException, MaskException, NumberFormatException,
            SignatureException {
        return doClustering(seq, DEFAULT_NUM_CLUSTERS, DEFAULT_NUM_ITERATIONS, 
                          DEFAULT_STABILITY_THRESHOLD, DEFAULT_COLOR_SPACE);
    }
     ...
     */
    public static Segmentation doClustering(Sequence seq, int numClusters)
            throws SupportRegionException, SegmentationException, MaskException, NumberFormatException,
            SignatureException {
        if (numClusters <= 0) {
            throw new IllegalArgumentException("Number of clusters must be positive: " + numClusters);
        }
        return doClustering(seq, numClusters, DEFAULT_NUM_ITERATIONS, 
                          DEFAULT_STABILITY_THRESHOLD, DEFAULT_COLOR_SPACE);
    }
}
