...
/**
 * Utility class for creating and managing cage spot charts.
 * This class provides functionality to build XY plots from cage data,
 * including data extraction, plot configuration, and rendering setup.
 * 
 * <p>ChartCageSpots handles the conversion of cage data into chart-ready
 * formats, manages global min/max values for axis scaling, and configures
 * plot appearance based on data characteristics.</p>
 * 
 * <p>Usage example:
 * <pre>
...
public class ChartCageSpots {
    ...
    private static final Logger LOGGER = Logger.getLogger(ChartCageSpots.class.getName());
    ...
    /**
     * Initializes the global min/max tracking variables.
     * This method should be called before processing new data to reset
     * the global extrema tracking.
     */
    ...
    /**
     * Combines results from two data arrays for a specific cage.
     * 
     * @param cage the cage to get data for
     * @param xlsResultsArray the primary data array
     * @param xlsResultsArray2 the secondary data array (can be null)
     * @return combined XYSeriesCollection containing all data for the cage
     * @throws IllegalArgumentException if cage or xlsResultsArray is null
     */
    public XYSeriesCollection combineResults(Cage cage, XLSResultsArray xlsResultsArray, XLSResultsArray xlsResultsArray2) {
        if (cage == null) {
            throw new IllegalArgumentException("Cage cannot be null");
        }
        if (xlsResultsArray == null) {
            throw new IllegalArgumentException("Primary results array cannot be null");
        }
        
        LOGGER.fine("Combining results for cage ID: " + cage.prop.cageID);
        
        XYSeriesCollection xyDataSetList = getSpotDataFromOneCage(xlsResultsArray, cage, "");
        
        if (xlsResultsArray2 != null) {
            XYSeriesCollection xyDataSetList2 = getSpotDataFromOneCage(xlsResultsArray2, cage, SECONDARY_DATA_TOKEN);
            addXYSeriesCollection(xyDataSetList, xyDataSetList2);
        }
        
        LOGGER.fine("Combined " + xyDataSetList.getSeriesCount() + " series for cage ID: " + cage.prop.cageID);
    ...
    /**
     * Updates the plot background and grid colors based on the number of flies in the data.
     * 
    ...
    private String buildSeriesDescription(XLSResults xlsResults, Cage cage) {
        return "ID:" + xlsResults.cageID + 
               ":Pos:" + xlsResults.cagePosition + 
               ":nflies:" + cage.prop.cageNFlies + 
               ":R:" + xlsResults.color.getRed() + 
               ":G:" + xlsResults.color.getGreen() + 
    ...
    private XYSeries getXYSeries(XLSResults results, String name) {
        ...
        if (results.valuesOut != null && results.valuesOut.length > 0) {
            ...
            LOGGER.fine("Created series '" + name + "' with " + results.valuesOut.length + " points");
        } else {
    ...
    private void updateGlobalMaxMin() {
        if (!flagMaxMinSet) {
            ...
            globalXMax = xmax;
            flagMaxMinSet = true;
            LOGGER.fine("Set initial global extrema: Y[" + globalYMin + ", " + globalYMax + "], X[0, " + globalXMax + "]");
        } else {
            ...
            if (updated) {
                LOGGER.fine("Updated global extrema: Y[" + globalYMin + ", " + globalYMax + "], X[0, " + globalXMax + "]");
            }
        }
    }
    ...
    private void addPointsAndUpdateExtrema(XYSeries seriesXY, XLSResults results, int startFrame) {
        if (seriesXY == null || results == null || results.valuesOut == null) {
    ...
    private XYLineAndShapeRenderer getSubPlotRenderer(XYSeriesCollection xySeriesCollection) {
        ...
        Stroke stroke = new BasicStroke(DEFAULT_STROKE_WIDTH, 
