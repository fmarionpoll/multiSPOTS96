...
/**
 * Extended polyline class for 2D level operations and data manipulation.
 * This class extends Icy's Polyline2D and provides additional functionality
 * for working with level data, including resizing, mathematical operations,
 * and specialized data processing methods.
 * 
 * <p>The Level2D class is particularly useful for:
 * <ul>
...
public class Level2D extends Polyline2D {
    ...
    /**
     * Creates a Level2D with the specified number of points.
     * X-coordinates are automatically set to sequential values (0, 1, 2, ...).
     * 
     ...
     * @throws IllegalArgumentException if arrays are null, numPoints is invalid, or arrays are too small
     */
    ...
    /**
     * Adds Y-coordinates from another Level2D to this one.
     * Expands the arrays if necessary to accommodate the source data.
     * 
    ...
    /**
     * Applies a threshold to all Y-coordinates.
     * Values greater than the threshold are set to 1, others to 0.
     * 
    ...
    /**
     * Computes the polarization index (PI) from two data sources.
     * PI = (data1 - data2) / (data1 + data2)
     * 
    ...
    /**
     * Computes a binary presence indicator from two data sources.
     * Result is 1 if either data source has a value > 0, otherwise 0.
     * 
     * @param data1 the first data source
     * @param data2 the second data source
     * @throws IllegalArgumentException if either data source is null
