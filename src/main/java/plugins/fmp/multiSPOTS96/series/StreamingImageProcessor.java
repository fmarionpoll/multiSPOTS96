 ...
 * 
 * This class provides efficient image loading and processing by loading
 * images on-demand rather than loading the entire stack into memory.
 */
public class StreamingImageProcessor {
    ...
    public IcyBufferedImage getImage(int frameIndex) {
        ...
        System.out.println("Loading frame " + frameIndex + " sequentially");
        ...
        if (image == null) {
            ...
            try {
                ...
                if (data == null) {
                    System.out.println(
                            "Native I/O created invalid image, falling back to Icy's imageIORead for frame "
                                    + frameIndex);
                    image = imageIORead(fileName);
                } else {
                ...
                System.out.println("Error validating native I/O image, falling back to Icy's imageIORead for frame "
                        + frameIndex);
                image = imageIORead(fileName);
            }
        }
        ...
        if (image != null) {
            ...
            if (memoryDelta > 10 || estimatedFootprint > 1) { // Log if significant memory increase or large
                ...
                System.out.println("Frame " + frameIndex + " loaded: " + memoryDelta + "MB memory increase "
                        + "(expected ~0.27MB). Estimated footprint: " + estimatedFootprint + "MB. "
                        + "Total memory: " + memoryAfter + "MB");
            }
