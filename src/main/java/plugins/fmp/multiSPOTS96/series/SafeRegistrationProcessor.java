...
public class SafeRegistrationProcessor implements RegistrationProcessor {
    ...
    @Override
    public ProcessingResult<RegistrationResult> correctDriftAndRotation(Experiment experiment, RegistrationOptions options) {
        return performRegistration(experiment, options, true, true);
    }
    
    @Override
    public ProcessingResult<TranslationResult> findTranslation(IcyBufferedImage sourceImage, IcyBufferedImage targetImage, int channel) {
        if (sourceImage == null) {
    ...
    public ProcessingResult<RotationResult> findRotation(IcyBufferedImage sourceImage, IcyBufferedImage targetImage, 
        ...
        try {
            Vector2d translation = previousTranslation != null ? 
                new Vector2d(previousTranslation.getX(), previousTranslation.getY()) : null;
        ...
        try {
            ...
            for (int frame = fromFrame; frame < toFrame && !stopFlag; frame++) {
                ...
                ProcessingResult<RegistrationResult> frameResult = processFrame(experiment, frame, referenceImage, 
                                                                             reducedReference, options, correctRotation, 
                                                                             correctTranslation, translationThreshold, 
                                                                             rotationThreshold);
    ...
    private ProcessingResult<RegistrationResult> processFrame(Experiment experiment, int frame, 
                                                            IcyBufferedImage referenceImage, 
                                                            IcyBufferedImage reducedReference,
                                                            RegistrationOptions options,
                                                            boolean correctRotation, boolean correctTranslation,
                                                            double translationThreshold, double rotationThreshold) {
        ...
        if (correctTranslation) {
            ProcessingResult<TranslationResult> translationResult = findTranslation(reducedWorkImage, reducedReference, options.getReferenceChannel());
            if (translationResult.isSuccess()) {
                TranslationResult translation = translationResult.getDataOrThrow();
                if (translation.isSignificant(translationThreshold)) {
                    ProcessingResult<IcyBufferedImage> appliedResult = applyTranslation(workImage, translation, -1, options.isPreserveImageSize());
        ...
        if (correctRotation) {
            ProcessingResult<RotationResult> rotationResult = findRotation(reducedWorkImage, reducedReference, options.getReferenceChannel(), null);
            if (rotationResult.isSuccess()) {
                ...
                if (rotation.isSignificant(rotationThreshold)) {
                    ProcessingResult<IcyBufferedImage> appliedResult = applyRotation(workImage, rotation, -1, options.isPreserveImageSize());
                    if (appliedResult.isSuccess()) {
                        workImage = appliedResult.getDataOrThrow();
        ...
        if (options.isSaveCorrectedImages() && (frameStats.getTotalTranslations() > 0 || frameStats.getTotalRotations() > 0)) {
    ...
    private static class RegistrationStatistics {
        ...
        public void incrementFramesCorrected() { framesCorrected++; }
        public void incrementTranslations() { totalTranslations++; }
        public void incrementRotations() { totalRotations++; }
        ...
        public RegistrationResult buildResult() {
            ...
            return new RegistrationResult(framesProcessed, framesCorrected, totalTranslations, 
                                       totalRotations, avgTranslation, avgRotation);
        }
    }
