...
/**
 * Configuration options for image registration operations.
 * Encapsulates all parameters needed for registration processing.
 */
public class RegistrationOptions {
    ...
    // Getters
    public int getFromFrame() { return fromFrame; }
    public int getToFrame() { return toFrame; }
    public int getReferenceFrame() { return referenceFrame; }
    public double getTranslationThreshold() { return translationThreshold; }
    public double getRotationThreshold() { return rotationThreshold; }
    public ImageTransformOptions getTransformOptions() { return transformOptions; }
    public boolean isSaveCorrectedImages() { return saveCorrectedImages; }
    public boolean isPreserveImageSize() { return preserveImageSize; }
    public int getReferenceChannel() { return referenceChannel; }
    public ProgressReporter getProgressReporter() { return progressReporter; }
    ...
    public ProcessingResult<Void> validate() {
        ...
        if (referenceFrame < fromFrame || referenceFrame > toFrame) {
            return ProcessingResult.failure("Reference frame must be within frame range [%d, %d], got: %d", 
                                         fromFrame, toFrame, referenceFrame);
        }
        
        if (translationThreshold < 0) {
            return ProcessingResult.failure("Translation threshold must be non-negative, got: %f", translationThreshold);
        }
    ...
    public String toString() {
        return String.format("RegistrationOptions{fromFrame=%d, toFrame=%d, referenceFrame=%d, " +
